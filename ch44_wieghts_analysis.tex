\section{Weights analysis} \label{chap:weight:analysis}
    ...
    We present the $10$ most originally underweighted and the $10$ most originally overweighted words for combinations of CR and TREC-DESC datasets and weighting schemes \textt{None} (no scheme was used), \textt{tfidf} (basic unsuperwised scheme) and \textt{tfig} (supervised scheme).
...
\end{table}
    ...
    In table \ref{tab:words:CR:tfidf} we see results for \textt{tfidf}. 
    Recall from section \ref{sec:term:weights}, that \textt{tfidf} decreases importance of words, that are common in the dataset.
...
\end{table}
    ...
    If we use just the term vectors (scheme \textt{None}), there are again obvious words that we underweight.
...
\end{table}
    ...
    In table \ref{tab:words:TREC:tfig} we see, that even supervised scheme \textt{tfig} underweighted some interrogative pronouns like \texttt{what}.
    ...
    On graph \ref{obr:hist:cr} we see, that \textt{tfidf} and \textt{None} schemes generally overweight on the CR dataset.
    We see \textt{tfig} produced low values of $w'$ less times, than \textt{None} and \textt{tfidf}.
